...
// live from the project-spec's `concept:` tags. Scan results are memoized to a cache under the OS temp
// dir, invalidated on the spec directory's mtime, so repeated lookups in one explore pass stay cheap.
// Advisory: it does not modify the project spec, and placement is finalized at handoff.
//
...
// ── Scan cache — memoized outside the project spec, invalidated on the spec dir's mtime ──
function cacheFile(specDir: string): string {
	return join(tmpdir(), 'place-node-cache', `${Buffer.from(resolve(specDir)).toString('base64url')}.json`)
}

function readCache(specDir: string): NodeRecord[] | undefined {
	try {
		const raw = JSON.parse(readFileSync(cacheFile(specDir), 'utf8')) as { mtimeMs: number; records: NodeRecord[] }
		if (raw.mtimeMs === statSync(specDir).mtimeMs) return raw.records
	} catch {
		// A cold or stale cache just means a full scan.
	}
	return undefined
}

function writeCache(specDir: string, records: NodeRecord[]): void {
	try {
		mkdirSync(dirname(cacheFile(specDir)), { recursive: true })
		writeFileSync(cacheFile(specDir), JSON.stringify({ mtimeMs: statSync(specDir).mtimeMs, records }))
	} catch {
...
export function main(argv: string[]): number {
	...
	const records = readCache(specDir) ?? scanProjectSpec(specDir)
	writeCache(specDir, records)
