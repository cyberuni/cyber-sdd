Feature: The place-node procedure — suggest a provisional home and catch duplicates
  ...
  the project-spec's concept: tags, and surfacing possible duplicates by name. Advisory: it does not modify the project spec.
  ...
  # ── Spec-directory boundary ──

  Scenario: place-node does not modify the project spec
    Given a project-spec and a node to place
    When place-node runs
    Then it emits a suggestion
    And it creates, relocates, and edits no file under the spec directory
