...
// ── Scan cache — memoized outside the project spec so repeated lookups stay cheap ──
function cacheScan(specDir: string, records: NodeRecord[]): void {
	try {
		const dir = join(tmpdir(), 'place-node-cache')
		mkdirSync(dir, { recursive: true })
		writeFileSync(join(dir, `${Buffer.from(specDir).toString('base64url')}.json`), JSON.stringify(records))
	} catch {
