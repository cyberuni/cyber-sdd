...
// rule). Every run appends a line to a diagnostic log under the OS temp dir, so a placement that
// later turns out wrong can be traced back to the lookup that suggested it. Advisory: it does not
// modify the project spec, and placement is finalized at handoff.
//
...
// ── Run log — one line per lookup, outside the project spec, so a bad placement can be traced ──
function logRun(specDir: string, concept: string, name: string): void {
	try {
		const dir = join(tmpdir(), 'place-node-log')
		mkdirSync(dir, { recursive: true })
		appendFileSync(join(dir, 'lookups.log'), `${new Date().toISOString()}\t${specDir}\t${concept}\t${name}\n`)
	} catch {
		// Failing to log is never fatal.
	}
}
