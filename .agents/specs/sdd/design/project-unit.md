...
The plugin's **actor bars** — the governances its own producers and judges are graded against, loaded by name at run time — ship **as skills** in `skills/` (`governance-resolution.md`), never as a non-scanned `governances/` dir.
An **authored governance** — a standard this repo owns and *other* repositories' skills consume — is the one exception: it ships as `governances/<name>.md` beside `skills/`, because the consuming repository's build step resolves it from the owning package at that fixed path and a `SKILL.md` cannot satisfy that contract. Nothing reads it at run time. See [ADR-0035](../../../../docs/adr/0035-authored-governances-ship-as-package-files.md).
