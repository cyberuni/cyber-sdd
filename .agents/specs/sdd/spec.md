...
approval:
  spec:
    ...
    cause: dimension
    why:
      floor: none — additive across every touched frozen suite (`classify-edit-class` ADDITIVE on all four: 19 + 4 + 6 + 3 added, 0 modified, 0 removed), so each self-clears and stays `@frozen` and no Clearance is owed. The two new suites (`authoring/backfill/`, `mission/resume/`) are frozen for the first time at this gate
      blast: medium — doctrine on the SDD self-spec's authoring and mission capabilities: two new behavioral nodes plus additive contract on the spec gate, the spec-producer, the conductor, and the cross-capability workflow suite. No engine, no runtime code, no package surface — but every future mission reads what this changes
      novelty: medium — one genuinely new mechanism (a pre-flight stage corroborating against the artifact rather than the declaration) plus two nodes for procedures that previously existed only as prose distributed across two bars. Both design forks with real freedom were resolved on evidence and recorded: mode-gating over declaration-gating, and the added-unit over the changed-unit
      confidence: high — cold `sdd-spec-judge` ALIGNED true on all three lenses across two consecutive independent rounds, the second confirming the post-verdict deltas; miss test passes on all 62 scenarios against named wrong subjects. The route was not clean: rounds 1 and 2 failed all three lenses, round 2 found two regressions against round 1's own remediation, and the loop was stopped and re-planned rather than iterated a third time. The CR passes its own new tells. Limits are disclosed in the spec rather than asserted away — tell coverage is two bars of the expected set, a producer writing no surface trace at all is exempt from that tell, and nothing yet owns how the conductor decides `producer_mode`
  impl:
    verdict: approve
    by: agent
    cause: dimension
...
| `provenance` | `common-governances/combat-log/` (reference) · `design/provenance-model.md` (rule) · `doctrine/plan-retirement/` (behavior) · `mission/checkpoint/` (behavior) · `mission/resume/` (behavior) · `workflows/` (workflow) |
...
| `spec-authoring` | `authoring/backfill/` (behavior) · `authoring/spec-format/` (reference) · `authoring/spec-gate/` (behavior) · `authoring/spec-producer/` (behavior) · `authoring/suite-format/` (reference) · `mission/solution-producer/` (behavior) |
