...
| **backfill** | behavioral | [`backfill/`](./backfill/README.md) | the ordered backfill workflow — five steps, each emitting a named output collected into one **step record**, plus the re-derivation that answers a `change` verdict against a node it produced; entered by the spec-producer's `backfill` mode |
